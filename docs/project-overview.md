...
当前主要应用工程，包含后端、前端、数据库迁移、开发脚本，以及 B站粉丝数、直播间、直播弹幕、直播场次/事件 XLSX/CSV/ZIP 导出和指定用户聚合工作台相关实现。
...
- B站直播场次边界、受支持事件留存、身份/金额统计和弹幕/礼物/用户 CSV 或完整 ZIP 导出。
...
`social-data-monitor/` 已经有可运行的 B站粉丝趋势监控、直播间监控、登录后自动升级的直播弹幕连接、可配置自动刷新的场次统计与 XLSX/CSV/ZIP 导出、指定用户聚合工作台。下次工作应在现有 Spring Boot 服务、Flyway 表结构、Vue 页面和 API 封装上继续小步迭代，不要推翻重写。
